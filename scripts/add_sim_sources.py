...
def create_sources(api_base: str, sim_base: str, env_ids: List[str]) -> None:
    sources = _get_sources(api_base)

    created = 0
    skipped = 0
    for env_id in env_ids:
        env_base = f"{sim_base.rstrip('/')}/env/{env_id}"
        payloads = [
            {
                "name": f"sim-scom-{env_id}",
                "type": "scom",
                "enabled": True,
                "config": {
                    "base_url": env_base,
            ...
            {
                "name": f"sim-thousandeyes-{env_id}",
                "type": "thousandeyes",
                "enabled": True,
                "config": {
                    "base_url": env_base,
...
def main() -> None:
    ...
    parser.add_argument("--env-ids", default=_env("SIM_ENV_IDS", "env-001,env-002,env-003"), help="Comma-separated env IDs")
    args = parser.parse_args()
    env_ids = [e.strip() for e in args.env_ids.split(",") if e.strip()]
    create_sources(args.api_base, args.sim_base, env_ids)
