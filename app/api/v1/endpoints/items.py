...
async def create_item(*, db: AsyncSession = Depends(get_db_session), item_in: ItemCreate) -> Any:
    """Create new item."""
    return await crud_item.create(db, obj_in=item_in)


@router.get("/{item_id}", response_model=Item)
async def read_item(*, db: AsyncSession = Depends(get_db_session), item_id: int) -> Any:
    """Get item by ID."""
...
async def delete_item(*, db: AsyncSession = Depends(get_db_session), item_id: int) -> Any:
    """Delete an item."""
